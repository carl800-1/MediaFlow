...
class BrushTask(object):
    ...
    _brush_tasks: Dict[str, Dict[str, Any]] = {}
    _torrents_cache: List[Any] = []
    _torrents_free_limit_cache: List[Any] = []
    _qb_client: str = "qbittorrent"
    _tr_client: str = "transmission"
    ...
    def get_brushtask_info(self, taskid: Optional[str] = None) -> Union[Dict[str, Any], Dict[str, Dict[str, Any]]]:
        """
        读取刷流任务列表
        """
        self.load_brushtasks()
    ...
    def check_task_rss(self, taskid: str) -> None:
        """
        检查RSS并添加下载，由定时服务调用
        """
        if not taskid:
    ...
    def __is_torrent_size_match(self, taskinfo, torrent_size=None):
        ...
        log.debug("【Brush】刷流任务 %s 当前保种体积 %sGB" % (task_name, round(int(total_size) / (1024 ** 3), 1)))
        ...
        if seed_size:
            ...
            log.warn("seed_size not configuration")
